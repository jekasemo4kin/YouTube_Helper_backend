...
$dotenv->load();
// $_ENV и $_SERVER это две суперглобальные переменные. 2 строки выше вытягивают инфу из файла .env с инфой и помещают в глобальную переменную $_ENV (Переменные окружения). 
// Dotenv\Dotenv::createImmutable это статический метод класса Dotenv\Dotenv, метод говорит искать файл .env в переменной __DIR__, а в этой крутой переменной хранится путь до файла, где эта переменная вызывается
// $_SERVER (Информация о сервере и выполнении) - это массив, содержащий инфу о заголовках, путях и расположении скрипта, также содержит переменные окружения, которые передаются веб-сервером 
// php и веб-сервер автоматом заполняют этот массив при каждом http - запросе 
